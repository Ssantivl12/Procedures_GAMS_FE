    ...
    <div class="space-y-6 animate-fade-in">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p class="text-xs text-muted-foreground mt-1">Feriados nacionales y municipales que no cuentan como días hábiles.</p>
        </div>
        
        <div class="flex items-center gap-3" *ngIf="!isReadOnly">
            <button (click)="showBulkModal = true" 
                    class="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground text-xs font-bold rounded-xl shadow-lg shadow-primary/20 hover:bg-primary/90 hover:scale-[1.02] active:scale-[0.98] transition-all">
        ...
        @for (day of nonWorkingDays(); track day.id) {
          <div class="bg-card p-4 rounded-2xl border border-border shadow-sm flex items-center justify-between group hover:border-primary/30 transition-all hover:translate-y-[-2px]">
            ...
            <button *ngIf="!isReadOnly" (click)="deleteDay(day.id)"
                    class="p-2 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 opacity-0 group-hover:opacity-100 transition-all">
      ...
      <div *ngIf="showBulkModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in" (click)="closeModal()">
        <div class="bg-card w-full max-w-lg rounded-3xl border border-border overflow-hidden shadow-2xl animate-scale-up" (click)="$event.stopPropagation()">
            <div class="p-6 border-b border-border bg-muted/30 flex justify-between items-center">
                ...
                <button (click)="closeModal()" class="p-2 hover:bg-muted rounded-xl transition-colors text-muted-foreground">
            ...
            <div class="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <div class="bg-primary/5 border border-primary/10 rounded-2xl p-4 mb-4">
                    <div class="flex gap-3">
                        <svg class="w-5 h-5 text-primary shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        <p class="text-xs text-primary font-medium leading-relaxed">
                            Ingrese las fechas y descripciones. Los duplicados serán omitidos automáticamente.
                        </p>
                    </div>
                </div>
                    ...
                    @for (row of bulkRows; track $index) {
                        <div class="flex gap-2 items-start animate-fade-in">
                            <input type="date" [(ngModel)]="row.date" 
                                   class="flex-1 px-3 py-2 text-xs font-bold rounded-xl border border-border bg-card focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                            ...
                            <button (click)="removeBulkRow($index)" class="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg shrink-0">
                ...
                <button (click)="addBulkRow()" class="w-full py-3 border-2 border-dashed border-border rounded-2xl text-xs font-bold text-muted-foreground hover:border-primary/50 hover:text-primary transition-all flex items-center justify-center gap-2">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                    Agregar otra fila
                </button>
            </div>
            ...
            <div class="p-6 bg-muted/30 border-t border-border flex justify-end gap-3">
                <button (click)="closeModal()" class="px-5 py-2.5 text-xs font-bold text-muted-foreground hover:bg-muted rounded-xl transition-all">Cancelar</button>
                <button (click)="submitBulk()" [disabled]="isSubmitting || !hasValidRows"
                        class="px-6 py-2.5 bg-primary text-primary-foreground text-xs font-bold rounded-xl shadow-lg shadow-primary/20 hover:bg-primary/90 disabled:opacity-50 flex items-center gap-2 transition-all">
...
export class NonWorkingDaysConfigComponent {
  ...
  bulkRows: { date: string, description: string }[] = [
    { date: '', description: '' }
  ];
  ...
  addBulkRow() {
    this.bulkRows.push({ date: '', description: '' });
  }
  ...
  closeModal() {
    if (!this.isSubmitting) {
        ...
        this.bulkRows = [{ date: '', description: '' }];
    }
  }
  ...
  deleteDay(id: string) {
    if (confirm('¿Está seguro de eliminar este día no laborable?')) {
