    ...
    <div class="flex items-center gap-3 p-3 rounded-lg hover:bg-muted/30 transition-colors group">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
          ...
          <span>Por: <span class="font-medium text-foreground">{{ getUploaderName() }}</span></span>
          <span class="text-gray-400">•</span>
          <span>el {{ (doc.uploadedAt) | date:'dd/MM/yyyy HH:mm' }}</span>
          <span class="text-gray-400">•</span>
      ...
      <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button class="cursor-pointer p-1.5 rounded-lg hover:bg-muted transition-colors" title="Descargar"
        ...
        @if (canDelete) {
          <button class="cursor-pointer p-1.5 rounded-lg hover:bg-red-50 transition-colors" title="Eliminar"
...
export class DocumentItemComponent {
  ...
  getUploaderName(): string {
    return (this.doc as any).uploadedBy?.fullName || 'Sistema';
  }
