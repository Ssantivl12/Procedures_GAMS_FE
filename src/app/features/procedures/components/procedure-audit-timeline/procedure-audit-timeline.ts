          ...
          <div class="space-y-4">
            @for (item of auditItems; track item.id) {
              <div class="relative flex gap-4 pl-8">
                <div class="absolute left-1.5 top-1.5 w-3 h-3 rounded-full bg-primary border-2 border-card"></div>
                <div class="flex-1 min-w-0">
                  ...
                  @if (item.note) {
                    <p class="text-xs text-muted-foreground mt-1 italic">{{ item.note }}</p>
                  }
                </div>
              </div>
            }
