        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex items-center gap-2">
            ...
            <select [(ngModel)]="category" (change)="onFilterChange()"
              ...
              <option value="C3">Industria C3</option>
              <option value="C4">Industria C4</option>
            </select>
          </div>

          <!-- Soft Delete (Superadmin only) -->
          @if (isSuperAdmin) {
            <label class="flex items-center gap-2 cursor-pointer group px-2 py-1 rounded-lg hover:bg-muted transition-colors">
              <input type="checkbox" [(ngModel)]="showInactive" (change)="onFilterChange()" 
                     class="w-3.5 h-3.5 rounded border-border text-primary focus:ring-primary/30 transition-all cursor-pointer">
              <span class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider group-hover:text-foreground transition-colors">Ver Inactivos</span>
            </label>
          }
          ...
          <button
            (click)="refresh.emit()"
            class="p-2 rounded-xl border border-border bg-card hover:bg-muted transition-colors group"
            title="Actualizar">
