          ...
          <form [formGroup]="form" (ngSubmit)="onSubmit()" class="p-6 space-y-4">
            ...
            <div *ngIf="!isEditing && !companyId">
              ...
              <div class="relative">
                ...
                <div *ngIf="isSearching || foundCompanies.length > 0" 
                    ...
                    <button type="button" *ngFor="let company of foundCompanies" 
                            ...
                            class="cursor-pointer w-full text-left px-4 py-2 text-sm hover:bg-muted transition-colors border-b border-border last:border-0">
              ...
              <div *ngIf="selectedCompany" class="mt-2 p-2 rounded-lg bg-primary/5 border border-primary/10 flex items-center justify-between">
                ...
                <button type="button" (click)="clearSelection()" class="cursor-pointer text-muted-foreground hover:text-destructive p-1">
...
export class CaseFileFormComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (this.isEditing && this.caseFileToEdit) {
      // For now, if we removed fileNumber, what are we updating?
      // If there are no other editable fields, we might not need updateCaseFile call here
      // but I will leave it empty for now or just skip if no fields.
      // The user wants to remove fileNumber completely.
      this.handleSuccess();
    } else {
